...
local MAGIC_TRY = 1
local DEFUALT_KEEPALIVE_TIMEOUT = 0
local DEFAULT_KEEPALIVE_CONS = 1000
local DEFUALT_TIMEOUT = 0

local function _do_retry(self, host, port, cmd, key, ...)
    local config = self.config
    local redis_client = redis:new()
    redis_client:set_timeout(config.timeout or DEFUALT_TIMEOUT)
        ...
        for i = 1, #serv_list do
            ...
            if ok then
                ...
                if err and string.sub(err, 1, 5) == "MOVED" then
                    local m, err = ngx.re.match(err, "MOVED [0-9]+ ([0-9.]+):([0-9]+)")
                    if type(m) == "table" then
                        res, err = _do_retry(self, m[1], m[2], cmd, key, ...)
--                        pr(m,cmd,key)
                    end
                    --                    self:fetch_slots()
                    --                    break
...
function _M.commit_pipeline(self)
    ...
    for k, v in pairs(map) do
        ...
        local ins = redis:new()
        ins:set_timeout(config.timeout or DEFUALT_TIMEOUT)
